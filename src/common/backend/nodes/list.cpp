...
static DList* new_dlist(NodeTag type)
{
    ...
    /* new_head->data is left undefined! */
    newHead = (DListCell*)palloc(sizeof(*newHead));
    newHead->next = NULL;
    newHead->prev = NULL;

    newList = (DList*)palloc(sizeof(*newList));
    newList->type = type;
    newList->length = 1;
    newList->head = newHead;
    newList->tail = newHead;
...
static void new_dlist_tail_cell(DList* dlist)
{
    ...
    new_tail = (DListCell*)palloc(sizeof(*new_tail));
    new_tail->next = NULL;
...
List *
list_delete_name(List *list, const char* name)
{
	ListCell   *cell = NULL;
	ListCell   *prev = NULL;
	check_list_invariants(list);
...
void dlist_add_tail_cell(DList* dlist, DListCell* cell)
{
    ...
    if (dlist == NULL || dlist->length < 1 || cell == NULL) {
        return;
    }
 ...
 */
bool list_is_subset(List* list1, List* list2)
{
    ListCell* cell = NULL;
