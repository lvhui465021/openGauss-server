...
DECLARE
    ans boolean;
BEGIN
select case when count(*)=1 then true else false end from (select * from pg_catalog.pg_conversion where conname = 'gb18030_2022_to_utf8' limit 1) into ans;
    if ans = true THEN
...
DECLARE
    ans boolean;
BEGIN
select case when count(*)=1 then true else false end from (select * from pg_catalog.pg_conversion where conname = 'utf8_to_gb18030_2022' limit 1) into ans;
    if ans = true THEN
